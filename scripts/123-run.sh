...
#set -x 
start_running_vm=$(virsh $CONNECTSTRING list | grep "vm-${vm_id}[[:space:]]" | grep -v clone | cut -d' ' -f2 )
for i in $start_running_vm ; do
 echo "Can't start running VM: vm-${vm_id} ID=$i"
 exit 0
done

start_stopped_vm=$(virsh $CONNECTSTRING list --all | grep "vm-${vm_id}[[:space:]]" | grep -v clone | awk  '{ print $2;}'  )
for i in $start_stopped_vm ; do
  echo "start VM: $i"
  virsh $CONNECTSTRING start $i
done
