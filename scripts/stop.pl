...
if ( $dom->is_active()) {
	$dom->shutdown();
	utils::yellow( "Shutdown vm-$vm_id:." );
	for ( my $i=0; $i < 120; $i++) {
	        utils::yellow( "." );
		sleep(2);
		last if not $dom->is_active();
	}

	utils::red( "\nCan't grasefuly shutdown VM: $vm_id. Do destroy." ) if $dom->is_active();
	$dom->destroy() if $dom->is_active();
	print "\n";
} else {
	utils::yellow( "Warning: vm-$vm_id already stopped...\n" );
}
